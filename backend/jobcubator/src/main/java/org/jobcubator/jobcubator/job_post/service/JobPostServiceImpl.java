...
public class JobPostServiceImpl implements JobPostService {
    ...
    public JobPostDTO createJobPost(
        ...
        if (
            !companySecurityService.canManageApplicationsAndJobPosts(
                companyId,
                user
            )
        ) {
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<JobPostDTO> filterJobPosts(
        JobPostFilterDTO filter,
        Pageable pageable
    ) {
        Specification<JobPost> spec = (root, query, cb) -> {
            ...
            if (filter != null) {
                ...
                if (filter.jobType() != null && !filter.jobType().isEmpty()) {
                    predicates.add(
                        cb.equal(root.get("jobType"), filter.jobType())
                    );
                }
                ...
                if (filter.tags() != null && !filter.tags().isEmpty()) {
                    ...
                    Join<JobPost, Tag> tagsJoin = root.join(
                        "tags",
                        JoinType.INNER
                    );
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<JobPostDTO> getJobPostsByCompanyId(
        UUID companyId,
        Pageable pageable
    ) {
        ...
        Page<JobPost> jobPostPage = jobPostRepository.findByCompany_Id(
            companyId,
            pageable
        );
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<JobPostDTO> getJobPostsByCompanyName(
        String companyName,
        Pageable pageable
    ) {
        Page<JobPost> jobPostPage =
            jobPostRepository.findByCompany_NameContainingIgnoreCase(
                companyName,
                pageable
            );
        return mapToJobPostDTO(jobPostPage);
    ...
    public Page<JobPostDTO> getTopVacanciesJobPosts(Pageable pageable) {
        Page<JobPost> page =
            jobPostRepository.findAllByOrderByNumberOfVacanciesDesc(pageable);
        return mapToJobPostDTO(page);
    }
    ...
    public Page<JobPostDTO> getRecentJobPosts(Pageable pageable) {
        Page<JobPost> page = jobPostRepository.findAllByOrderByCreatedAtDesc(
            pageable
        );
        return mapToJobPostDTO(page);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<JobPostDTO> getJobPostsByTagName(
        Pageable pageable,
        String tagName
    ) {
